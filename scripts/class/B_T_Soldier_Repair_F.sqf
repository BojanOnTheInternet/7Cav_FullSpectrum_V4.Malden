...
"You are a logistics specialist.  Your primary responsibility is to support armor operations by providing fuel, ammunition and repairs, and logistics HEMTTs are available for each type of support.  Only logistics specialists
may drive the logistics HEMTTs, but transport pilots can deliver the logistics pods with the Taru helicopters.  Those pods can make for quick resupply from base, especially when an operation is far from base.<br/><br/>
The building that you started in is the storehouse for ammunition on base.  The two trolleys, or pallet jacks, can be used to quickly move up to two tons of ammunition at a time.  Only logistics specialists may use the trolleys,
and only near the storehouse.  Without the trolleys, players are limited to moving only as much as they can carry, which is 65kg.<br/><br/>";

	private _roleRecord = player createDiaryRecord ["diary", ["Logistics specialist",
		_roleDescription + "<br/>" +
